module timed_tc_app_module
    ...
    type, extends(timed_application) :: timed_tc_app
        type(tc_app) :: app
    ...
    end type timed_tc_app

    interface timed_tc_app
        module procedure constructor
    end interface timed_tc_app

contains
    function constructor() result(this)
        type(timed_tc_app) :: this

        call this%clear()
    end function constructor

    subroutine initialize(this, config)
        class(timed_tc_app), intent(inout) :: this
        ...
        atimer = timer()
        priorities = [string("timed_tc-"), string("tc-")]

        call this%app%check_config(config, priorities)
        call this%initialize_metrics()

        call this%app%setup_tc(config, priorities)
        ...
        this%sizes = converter%to_int64_array(config%full_config%get_value("dims", priorities))
    ...
    subroutine set_allocation_metric(this, config, priorities)
        ...
        call this%add_gibi_metric(this%allocated_gibibytes, this%datatype_size, this%sizes, this%descriptor%c_indices)
        call this%add_gibi_metric(this%allocated_gibibytes, this%datatype_size, this%sizes, this%descriptor%a_indices)
        call this%add_gibi_metric(this%allocated_gibibytes, this%datatype_size, this%sizes, this%descriptor%b_indices)
    end subroutine set_allocation_metric

    subroutine add_gibi_metric(this, metric, datatype_size, sizes, indices)
        class(timed_tc_app), intent(in) :: this
    ...
    subroutine run(this)
        ...
        call this%add_gibi_metric(this%gibibytes_read, this%datatype_size, this%sizes, this%descriptor%a_indices)
        call this%add_gibi_metric(this%gibibytes_read, this%datatype_size, this%sizes, this%descriptor%b_indices)
        call this%add_gibi_metric(this%gibibytes_written, this%datatype_size, this%sizes, this%descriptor%c_indices)
...
end module timed_tc_app_module
